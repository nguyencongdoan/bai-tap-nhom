		...
		<header role="banner" class="page-header">
			<?php if ( have_posts() ) : ?>
				<h1 class="search-title">
					<?php /* translators: %s: search term */ printf( esc_html__( 'Search Results for: %s','the-writers-blog'), '<span>' . esc_html( get_search_query() ) . '</span>' ); ?></h1>
			<?php else : ?>
				<h1 class="page-title"><?php echo esc_html(get_theme_mod('the_writers_blog_no_search_result_title',__('Nothing Found','the-writers-blog')));?></h1>
			<?php endif; ?>
		</header>
		<div class="row">
			<?php
				...
				<div class="col-lg-8 col-md-8">
					<?php
						if ( have_posts() ) :
							...
							<p><?php echo esc_html(get_theme_mod('the_writers_blog_no_search_result_content',__('Sorry, but nothing matched your search terms. Please try again with some different keywords.','the-writers-blog')));?></p>
							<?php
								get_search_form();
